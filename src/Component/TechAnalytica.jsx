...
// const TOTALTALBALANCE=totalTal;
// console.log(TOTALTALBALANCE);
// console.log('Balance type', typeof(Balance), Balance);



const previousBalance=()=>{
  console.log(Balance,previous, totalTal, typeof(Balance),"Edit button clicked");
...
let totalfromState=totalTal;
if(typeof(totalfromState)=='string'){
  const totalTalBalance=totalfromState.slice(1, -1);
  totalfromState=parseInt(totalTalBalance);
 console.log(totalfromState);
}

  // console.log('Pre--', pre, totalfromState, Balance);
  // console.log(totalTal);
  let Total_tal=totalfromState+Balance+pre;
  // console.log('Previous balance nagetive', Total_tal);
  // if(Total_tal < 0){
  //   let result = Total_tal.toString().replaceAll("-","(").concat(")");
  //   console.log(typeof(result));
  //   Total_tal=result;
  //   set(ref(db, 'total_balance_tal'), {
  //     Total_tal
  //   });
  // }
  set(ref(db, 'total_balance_tal'), {
  ...
  let Total_tal=totalTal+Balance-previous;
...
// }
  ...
  const totalBalance=()=>{
    // console.log(totalTal, Balance);
   let Total_tal=totalTal+Balance;
  //  console.log(Total_tal);
  set(ref(db, 'total_balance_tal'), {
    Total_tal
  });
}
// const TOTALTALBALANCE=totalTal;
  ...
  useEffect(() => {
    onValue(ref(db), (snapshot) => {
      ...
      if (datas !== null) {
        Object.values(datas.user_tal).map((todo) => {
          setData((oldArray) => [...oldArray, todo]);
        });
      } 
      setTotalTal(datas.total_balance_tal.Total_tal);
    })
  }, [totalTal])
  ...
  const handleApprove = (row) => {
    update(ref(db, `user_tal/ +${row.original.uuid}`), {
      approve:true,
    });
  }
  ...
  const updateToDatabase = () => {
    ...
    update(ref(db, `user_tal/ +${tempUuid}`), {
      cParticular,
      todo1,
      todo2,
      todo3,
  ...
  const handleApprovedAll=(row)=>{
      row.map(item=>{
        ...
        update(ref(db, `user_tal/ +${item.original.uuid}`), {
          approve:true,
        });
      })
    }
  // const handleUpdateAll=(row)=>{
  //   // console.log("-----updaate");
  //     row.map(item=>{
  //       update(ref(db, `user_tal/ +${item.original.uuid}`), {
  //         date:item.original.date+"update",
  //       });
  //     })
  //   }
...
//   // console.log("starting2",starting2);
//   // console.log("ending2" ,ending2);
// var userInfoTal=[];
// const que=query(ref(db, "user_tal"), orderByChild("si"), startAt(parseInt(starting2)), endAt(parseInt(ending2)));
// get(que)
// .then((snapshot)=>{
// snapshot.forEach(childSnapshot =>{
//   userInfoTal.push(childSnapshot.val());
// // console.log(userInfoTal);
// })
// PdfFileGenate(userInfoTal);
// })
// setStarting2('');
// setEnding2('');
// }
...
//pdf section 
const PdfFileGenate=(userInfoTal)=>{
  userInfoTal.sort(function(a, b) {
    var keyA = a.si
     var keyB = b.si
    // Compare the 2 dates
  ...
  userInfoTal.push({"todo1":c_total, "todo4":d_total, "Balance":total_Balance})

  // const date = new Date().toString().slice(3,16);
  const doc = new jsPDF('l')
  doc.text("Tech analytica ltd account information", 20, 10)
  doc.autoTable({
    theme: "grid",
...
// console.log(data.);
  return (
    <Box marginTop={4} padding={2}>
        <Typography variant='h2' color={'primary.dark'}>TECH ANALYTICA LIMITED</Typography>
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6}>
            <Box>
              ...
              <TextField  margin='normal' label="Notes" name='notes' type={'text'} variant='outlined' value={todo7} onChange={handleTodoChange7} />
            </Box>
            </Grid>
            <Grid item xs={12} sm={6}>
            <Box>
    ...
    <TextField margin='2'
              ...
              inputProps={{
                ...
                onChange={(e)=>{
                  ...
                  // console.log("date picker ----", starting2, starting2);
    ...
    <>
      ...
      >
        ...
        {/* <Button
         disabled={table.getPrePaginationRowModel().rows.length === 0}
          sx={{bgcolor:'primary.dark', color:'white'}}
          onClick={()=>handleUpdateAll(table.getPrePaginationRowModel().rows)}
          variant="contained"
        >
          UPDATE ALL
        </Button> */}
